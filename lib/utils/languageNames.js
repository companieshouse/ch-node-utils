...
const subDirs_1 = tslib_1.__importDefault(require("./subDirs"));
class NamedIsoCode {
    IsoCode;
    Name;
...
class LanguageNames {
    ...
    static sourceLocales(localesDir) {
        const localesArray = subDirs_1.default.getSubDirs(localesDir);
        return LanguageNames.getNativeNamesObjectArray(localesArray.sort(LanguageNames.customSort));
    }
    static isSupportedLocale(localesDir, locale) {
        return subDirs_1.default.getSubDirs(localesDir).includes(locale);
    }
}
